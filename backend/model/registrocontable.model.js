...
class RegistrocontableModel {
    static async readRegistroContableModel(cuenta,fecha,fechaF,tipo)
    {
        try {
            ...
            var sql = "select RC.id_registros_contables,RC.fk_id_cuenta_bancaria,convert(date(RC.fecha),char(50)) fecha,RC.descripcion,RC.monto,RC.fk_id_tipo from registros_contables as RC where RC.monto > 0 "+sqlCuenta+sqlFecha+sqlTipo
